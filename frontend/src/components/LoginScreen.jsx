  ...
  const handleEmailAuth = async (e) => {
    ...
    try {
      if (isSignUp) {
        ...
        // Enhanced user data for email users
        const enhancedUser = {
          ...user,
    ...
    <>
      <div className="min-h-screen bg-gradient-to-br from-purple-900 via-blue-900 to-indigo-900 flex items-center justify-center p-4">
        ...
        >
          ...
          {/* Login Card - Improved responsive padding */}
          <div className="bg-white/10 backdrop-blur-md border border-white/20 rounded-2xl p-6 sm:p-8 shadow-2xl">
            ...
            <div ref={errorRef}>
              {error && (
                <div className="bg-red-500/20 border border-red-400/50 rounded-xl p-3 mb-4">
                  <p className="text-red-200 text-xs sm:text-sm text-center break-words">{error}</p>
                </div>
              )}

              {showGoogleHelp && (
                <div className="bg-blue-500/20 border border-blue-400/50 rounded-xl p-3 sm:p-4 mb-4">
                  <h4 className="text-blue-200 font-semibold mb-2 text-sm sm:text-base">Google Sign-In Issues?</h4>
                  ...
                  <div className="flex flex-col sm:flex-row space-y-2 sm:space-y-0 sm:space-x-2 mt-3">
                    <button
                      ...
                      className="flex-1 text-blue-300 hover:text-white text-xs sm:text-sm underline"
                    >
                      I've fixed it, try again
                    </button>
                    <button
                      onClick={handleShowTroubleshooting}
                      className="flex-1 bg-blue-600/30 hover:bg-blue-600/50 text-blue-200 text-xs sm:text-sm px-3 py-1 rounded"
                    >
                      Detailed Help
                    </button>
                  </div>
            ...
            {showEmailForm ? (
              /* Email Form - Improved responsive inputs */
              ...
              /* OAuth and Options - Improved mobile layout */
                ...
                <button
                  onClick={() => {
                    const quickDemoUser = {
                      ...
                      loginTimestamp: new Date().toISOString(),
                      preferences: {
                        adventureTypes: ['creativity', 'mindfulness'],
                        difficulty: 'medium'
                      }
                    };
          ...
          {import.meta.env.DEV && (
            <div className="mt-4 p-3 bg-black/30 rounded-xl text-xs text-white/60">
              ...
              <p>API Base: {import.meta.env.VITE_API_URL || 'Default'}</p>
